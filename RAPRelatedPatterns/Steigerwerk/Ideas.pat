...
MEANING "Identifiers MUST uniquely identify an entity."

GEN Namespace ISA Scope
hasSubnamespace :: Namespace * Namespace [INJ,ASY] PRAGMA "" " has " " as a subNamespace"
MEANING "A namespace hierarchy allows names to be derefenced even though they are not defined in the namespace itself.".

RULE hasSubnamespace |- hasSubscope
MEANING "Subnamespaces are subscopes, albeit hierarchical ones."
...
MEANING "For every pair (Namespace,Symbol), there exists one Idquery."

RULE allIdqueriesRepresentOnePair: I[Idquery] = namespace;namespace~ /\ symbol;symbol~
MEANING "For every Idquery, there exists one pair (Namespace,Symbol)."

identifies :: Idquery * Entity [UNI] PRAGMA "" " uniquely identifies "
MEANING "The entity that is identified by a symbol within a namespace does not need to be explicitly defined through an identifier of that namespace. It can also be inferred from its meaning in higher-level namespaces, as long as this meaning is unambiguous.".

RULE undereferenceable: I[Idquery] = identifies; identifies~
MEANING "This rule signals all identifiers that do not, or do no longer, refer to an entity."
...
MEANING "A symbol is dereferenceable within namespace n either if it is the name of an identifier in that namespace, or it is dereferenceable in a namespace that is a superscope."

RULE identifyuniquely: I[Idquery] = identifies;identifies~
MEANING "The result of dereferencing symbols must be unambiguous."
...
MEANING "Every concatenation is uniquely characterized by its first and second arguments."
...
MEANING "The context within which a service call operates includes a subcontext dedicated to a communications channel.".

sessionDomain :: Session -> Domain PRAGMA "Accountability for all session information disclosed through " " is assumed by "
MEANING "If a service call is run under accountability of an organization O1, but the service call is triggered by a request made by an organization O2, then O1 will be accountable for the service call, whereas O2 will be accountable for (the correctness and perhaps reliability of) all session information, a common example of which would be the user-identifier. Also, since any actor whose actions O2 is accountable for should be controlled by O2, it seems natural to have O2 assign things like permissions, roles and other attributes that the session discloses.".
...
MEANING "The identifier that identifies the actor that sends and receives messages to and from services in a session, is (at least) dereferenceable to that actor within session's domain. For example, if a session s4711 has a sessiondomain 'TNO' and a sessionUser identifier 'rieks.joosten@tno.nl', then within the domain 'TNO', this identifier can be dereferenced to identify the actor that exchanged messages with services in s4711".
