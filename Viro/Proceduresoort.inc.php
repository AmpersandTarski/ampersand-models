  ...
  class Proceduresoort {
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      foreach($me['Procedures'] as $i0=>$v0){
        if(isset($v0['id']))
          DB_doquer("UPDATE `procedur` SET `i`='".addslashes($v0['id'])."', `rechtsgebied`='".addslashes($v0['rechtsgebied'])."', `zorgdrager`='".addslashes($v0['zorgdrager dossier'])."' WHERE `i`='".addslashes($v0['Procedure'])."'", 5);
      }
