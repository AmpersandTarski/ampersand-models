...
\***************************************/
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['0.'.$i0]);$i0++){
      $faseHandeling[$i0] = array( 'id' => @$r['0.'.$i0.'.0']
                                 , 'Handeling' => @$r['0.'.$i0.'.0']
                                 );
      $faseHandeling[$i0]['door']=array();
      for($i1=0;isset($r['0.'.$i0.'.1.'.$i1]);$i1++){
        $faseHandeling[$i0]['door'][$i1] = @$r['0.'.$i0.'.1.'.$i1.''];
      }
      $faseHandeling[$i0]['prio']=array();
      for($i1=0;isset($r['0.'.$i0.'.2.'.$i1]);$i1++){
        $faseHandeling[$i0]['prio'][$i1] = @$r['0.'.$i0.'.2.'.$i1.''];
      }
      $faseHandeling[$i0]['usecase']=array();
      for($i1=0;isset($r['0.'.$i0.'.3.'.$i1]);$i1++){
        $faseHandeling[$i0]['usecase'][$i1] = @$r['0.'.$i0.'.3.'.$i1.''];
      }
      $faseHandeling[$i0]['rol']=array();
      for($i1=0;isset($r['0.'.$i0.'.4.'.$i1]);$i1++){
        $faseHandeling[$i0]['rol'][$i1] = @$r['0.'.$i0.'.4.'.$i1.''];
      }
    }
    $Fase=new Fase($ID,$faseHandeling);
    if($Fase->save()!==false) die('ok:'.$_SERVER['PHP_SELF'].'?Fase='.urlencode($Fase->getId())); else die('Please fix errors!');
  ...
  if(isset($_REQUEST['Fase'])){
    if(!$del || !delFase($_REQUEST['Fase']))
      $Fase = readFase($_REQUEST['Fase']);
    else $Fase = false; // delete was a succes!
  } else if($new) $Fase = new Fase();
  else $Fase = false;
  if($Fase){
    writeHead("<TITLE>Fase - VIRO - ADL Prototype</TITLE>"
              .($edit?'<SCRIPT type="text/javascript" src="edit.js"></SCRIPT>':'<SCRIPT type="text/javascript" src="navigate.js"></SCRIPT>')."\n" );
    if($edit)
        echo '<FORM name="editForm" action="'
              .$_SERVER['PHP_SELF'].'" method="POST" class="Edit">';
    if($edit && $Fase->isNew())
         echo '<P><INPUT TYPE="TEXT" NAME="ID" VALUE="'.addslashes($Fase->getId()).'" /></P>';
    else echo '<H1>'.$Fase->getId().'</H1>';
    ?>
    <DIV class="Floater fase Handeling">
      <DIV class="FloaterHeader">fase Handeling</DIV>
      <DIV class="FloaterContent"><?php
          $faseHandeling = $Fase->get_faseHandeling();
          echo '
          <UL>';
          foreach($faseHandeling as $i0=>$v0){
            echo '
          ...
          if($edit) echo '
            <LI CLASS="new UI" ID="0.'.count($faseHandeling).'">new fase Handeling</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <SCRIPT type="text/javascript">
      // code for editing blocks in fase Handeling
      function UI(id){
   ...
   if($edit){
     if($new) 
       ...
       $buttons.=ifaceButton("JavaScript:save('".$_SERVER['PHP_SELF']."?save=1','".urlencode($Fase->getId())."');","Save");
       $buttons.=ifaceButton($_SERVER['PHP_SELF']."?Fase=".urlencode($Fase->getId()),"Cancel");
     } 
  } else $buttons.=ifaceButton($_SERVER['PHP_SELF']."?edit=1&Fase=".urlencode($Fase->getId()),"Edit")
                 .ifaceButton($_SERVER['PHP_SELF']."?del=1&Fase=".urlencode($Fase->getId()),"Delete");
  }else{
    if($del){
      writeHead("<TITLE>Delete geslaagd</TITLE>");
      echo 'The Fase is deleted';
    }else{  // deze pagina zou onbereikbaar moeten zijn
      writeHead("<TITLE>No Fase object selected - VIRO - ADL Prototype</TITLE>");
      ?><i>No Fase object selected</i><?php 
    }
