...
\***************************************/
  ...
  require "RechterlijkeAmbtenaar.inc.php";
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['3.'.$i0]);$i0++){
      $geautoriseerdvoor[$i0] = @$r['3.'.$i0.''];
    }
    $Zittingen=array();
    for($i0=0;isset($r['4.'.$i0]);$i0++){
      $Zittingen[$i0] = array( 'id' => @$r['4.'.$i0.'']
                             , 'griffier' => @$r['4.'.$i0.'.1']
                             , 'geagendeerd' => @$r['4.'.$i0.'.2']
                             , 'plaats' => @$r['4.'.$i0.'.3']
                             , 'locatie' => @$r['4.'.$i0.'.4']
                             , 'kamer' => array( 'id' => @$r['4.'.$i0.'.5'], 'gerecht' => @$r['4.'.$i0.'.5.0'], 'sector' => @$r['4.'.$i0.'.5.1'])
                             );
      $Zittingen[$i0]['rechter']=array();
      for($i1=0;isset($r['4.'.$i0.'.0.'.$i1]);$i1++){
        $Zittingen[$i0]['rechter'][$i1] = @$r['4.'.$i0.'.0.'.$i1.''];
      }
    }
    $ontvangen=array();
    ...
    $RechterlijkeAmbtenaar=new RechterlijkeAmbtenaar($ID,$gerecht, $kamer, $rol, $geautoriseerdvoor, $Zittingen, $ontvangen, $verzonden);
  ...
  if($RechterlijkeAmbtenaar){
    writeHead("<TITLE>RechterlijkeAmbtenaar - VIRO - ADL Prototype</TITLE>"
    ...
    <DIV class="Floater gerecht">
      ...
      <DIV class="FloaterContent"><?php
          $gerecht = $RechterlijkeAmbtenaar->get_gerecht();
          echo '
          <UL>';
          foreach($gerecht as $i0=>$v0){
            echo '
            <LI CLASS="item UI_gerecht" ID="0.'.$i0.'">';
              if(!$edit) echo '
          ...
          if($edit) echo '
            <LI CLASS="new UI_gerecht" ID="0.'.count($gerecht).'">new gerecht</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <DIV class="Floater kamer">
      ...
      <DIV class="FloaterContent"><?php
          $kamer = $RechterlijkeAmbtenaar->get_kamer();
          echo '
          <UL>';
          foreach($kamer as $i0=>$v0){
            echo '
            <LI CLASS="item UI_kamer" ID="1.'.$i0.'">';
              if(!$edit){
                echo '
              ...
              <DIV>';
                echo 'kamer: ';
                echo '<SPAN CLASS="item UI_kamer_kamer" ID="1.'.$i0.'.0">';
                echo htmlspecialchars($v0['kamer']);
                echo '</SPAN>';
              echo '</DIV>
              <DIV>';
                echo 'sector: ';
                echo '<SPAN CLASS="item UI_kamer_sector" ID="1.'.$i0.'.1">';
                if(!$edit) echo '
          ...
          if($edit) echo '
            <LI CLASS="new UI_kamer" ID="1.'.count($kamer).'">new kamer</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <SCRIPT type="text/javascript">
      // code for editing blocks in kamer
      function UI_kamer(id){
        return '<DIV>kamer: <SPAN CLASS="item UI_kamer_kamer" ID="'+id+'.0"></SPAN></DIV>'
             + '<DIV>sector: <SPAN CLASS="item UI_kamer_sector" ID="'+id+'.1"></SPAN></DIV>'
              ;
      }
    </SCRIPT>
    <?php } ?>
    <DIV class="Floater rol">
      ...
      <DIV class="FloaterContent"><?php
          ...
          foreach($rol as $i0=>$v0){
            echo '
            <LI CLASS="item UI_rol" ID="2.'.$i0.'">';
              echo htmlspecialchars($v0);
            echo '</LI>';
          }
          if($edit) echo '
            <LI CLASS="new UI_rol" ID="2.'.count($rol).'">new rol</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <DIV class="Floater geautoriseerd voor">
      ...
      <DIV class="FloaterContent"><?php
          ...
          foreach($geautoriseerdvoor as $i0=>$v0){
            echo '
            <LI CLASS="item UI_geautoriseerdvoor" ID="3.'.$i0.'">';
              echo htmlspecialchars($v0);
            echo '</LI>';
          }
          if($edit) echo '
            <LI CLASS="new UI_geautoriseerdvoor" ID="3.'.count($geautoriseerdvoor).'">new geautoriseerd voor</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <DIV class="Floater Zittingen">
      ...
      <DIV class="FloaterContent"><?php
          ...
          foreach($Zittingen as $i0=>$v0){
            echo '
            <LI CLASS="item UI_Zittingen" ID="4.'.$i0.'">';
              if(!$edit){
                echo '
              ...
              <DIV>';
                ...
                foreach($v0['rechter'] as $i1=>$rechter){
                  echo '
                  <LI CLASS="item UI_Zittingen_rechter" ID="4.'.$i0.'.0.'.$i1.'">';
                    if(!$edit){
                      echo '
                    <A class="GotoLink" id="To4.'.$i0.'.0.'.$i1.'">';
                      echo htmlspecialchars($rechter).'</A>';
                      echo '<DIV class="Goto" id="GoTo4.'.$i0.'.0.'.$i1.'"><UL>';
                ...
                if($edit) echo '
                  <LI CLASS="new UI_Zittingen_rechter" ID="4.'.$i0.'.0.'.count($v0['rechter']).'">new rechter</LI>';
                echo '
                </UL>';
              echo '</DIV>
              <DIV>';
                echo 'griffier: ';
                echo '<SPAN CLASS="item UI_Zittingen_griffier" ID="4.'.$i0.'.1">';
                if(!$edit){
                  echo '
              ...
              <DIV>';
                echo 'geagendeerd: ';
                echo '<SPAN CLASS="item UI_Zittingen_geagendeerd" ID="4.'.$i0.'.2">';
                echo htmlspecialchars($v0['geagendeerd']);
                echo '</SPAN>';
              echo '</DIV>
              <DIV>';
                echo 'plaats: ';
                echo '<SPAN CLASS="item UI_Zittingen_plaats" ID="4.'.$i0.'.3">';
                if(!$edit) echo '
              ...
              <DIV>';
                echo 'locatie: ';
                echo '<SPAN CLASS="item UI_Zittingen_locatie" ID="4.'.$i0.'.4">';
                if(!$edit) echo '
              ...
              <DIV>';
                ...
                <DIV class ="Holder"><DIV class="HolderHeader">kamer</DIV>
                  <DIV class="HolderContent" name="kamer"><?php
                      echo '<DIV CLASS="UI_Zittingen_kamer" ID="4.'.$i0.'.5">';
                        if(!$edit){
                          echo '
                        ...
                        <DIV>';
                          echo 'gerecht: ';
                          echo '<SPAN CLASS="item UI_Zittingen_kamer_gerecht" ID="4.'.$i0.'.5.0">';
                          if(!$edit) echo '
                        ...
                        <DIV>';
                          echo 'sector: ';
                          echo '<SPAN CLASS="item UI_Zittingen_kamer_sector" ID="4.'.$i0.'.5.1">';
                          if(!$edit) echo '
                          <A HREF="Sector.php?Sector='.urlencode($v0['kamer']['sector']).'">'.htmlspecialchars($v0['kamer']['sector']).'</A>';
                          else echo htmlspecialchars($v0['kamer']['sector']);
                          echo '</SPAN>';
                        echo '
                        </DIV>';
                        if($edit) echo '
                        <INPUT TYPE="hidden" name="4.'.$i0.'.5.ID" VALUE="'.$v0['kamer']['id'].'" />';
                      echo '</DIV>';
                    ?> 
                  </DIV>
                </DIV>
          ...
          if($edit) echo '
            <LI CLASS="new UI_Zittingen" ID="4.'.count($Zittingen).'">new Zittingen</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <SCRIPT type="text/javascript">
      ...
      function UI_Zittingen(id){
        return '<DIV>rechter: <UL><LI CLASS="new UI_Zittingen_rechter" ID="'+id+'.0">new rechter</LI></UL></DIV>'
             ...
             + '<DIV>kamer: <SPAN CLASS="item UI_Zittingen_kamer" ID="'+id+'.5"><DIV>gerecht: <SPAN CLASS="item UI_Zittingen_kamer_gerecht" ID="'+id+'.50"></SPAN></DIV><DIV>sector: <SPAN CLASS="item UI_Zittingen_kamer_sector" ID="'+id+'.51"></SPAN></DIV></SPAN></DIV>'
              ;
      }
    </SCRIPT>
    <?php } ?>
   ...
   if($edit){
    ...
    if($del){
      ...
      echo 'The RechterlijkeAmbtenaar is deleted';
    }else{  // deze pagina zou onbereikbaar moeten zijn
      writeHead("<TITLE>No RechterlijkeAmbtenaar object selected - VIRO - ADL Prototype</TITLE>");
      ?><i>No RechterlijkeAmbtenaar object selected</i><?php 
    }
