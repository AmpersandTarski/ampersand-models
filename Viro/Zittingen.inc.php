  ...
  class Zittingen {
      ...
      if(!isset($Zittingen)){
        ...
        foreach($me['Zittingen'] as $i0=>&$v0){
          $v0=firstRow(DB_doquer("SELECT DISTINCT '".addslashes($v0['id'])."' AS `id`
                                       ...
                                       , `f3`.`locatie` AS `gerecht`
                                       , `f4`.`griffier`
                                       , `f5`.`geagendeerd`
                                       , `f6`.`plaatsgevonden` AS `feitelijkedatum`
                                    FROM `zitting`
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      foreach($me['Zittingen'] as $i0=>$v0){
        if(isset($v0['id']))
          DB_doquer("UPDATE `zitting` SET `i`='".addslashes($v0['id'])."', `locatie`='".addslashes($v0['gerecht'])."', `griffier`='".addslashes($v0['griffier'])."', `geagendeerd`='".addslashes($v0['geagendeerd'])."', `plaatsgevonden`=".((null!=$v0['feitelijkedatum'])?"'".addslashes($v0['feitelijkedatum'])."'":"NULL")." WHERE `i`='".addslashes($v0['nr'])."'", 5);
      }
