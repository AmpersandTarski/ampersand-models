<?php // generated with ADL vs. 0.8.10-452
  ...
  class nieuweProcedure {
    ...
    function nieuweProcedure($id=null, $zorgdrager=null, $rechtsgebied=null, $proceduresoort=null){
      $this->_id=$id;
      $this->_zorgdrager=$zorgdrager;
      $this->_rechtsgebied=$rechtsgebied;
      $this->_proceduresoort=$proceduresoort;
      if(!isset($zorgdrager) && isset($id)){
        ...
        if(count($ctx)==0) $this->_new=true; else
        {
          ...
          $this->set_zorgdrager($me['zorgdrager']);
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      $me=array("id"=>$this->getId(), "zorgdrager" => $this->_zorgdrager, "rechtsgebied" => $this->_rechtsgebied, "proceduresoort" => $this->_proceduresoort);
      DB_doquer("DELETE FROM `procedur` WHERE `i`='".addslashes($me['id'])."'",5);
      $res=DB_doquer("INSERT IGNORE INTO `procedur` (`zorgdrager`,`rechtsgebied`,`proceduresoort`,`i`) VALUES ('".addslashes($me['zorgdrager'])."', '".addslashes($me['rechtsgebied'])."', '".addslashes($me['proceduresoort'])."', '".addslashes($me['id'])."')", 5);
      if($newID) $this->setId($me['id']=mysql_insert_id());
      DB_doquer("DELETE FROM `orgaan` WHERE `i`='".addslashes($me['zorgdrager'])."'",5);
      $res=DB_doquer("INSERT IGNORE INTO `orgaan` (`i`) VALUES ('".addslashes($me['zorgdrager'])."')", 5);
    ...
    function set_zorgdrager($val){
      $this->_zorgdrager=$val;
    }
    function get_zorgdrager(){
      return $this->_zorgdrager;
    }
