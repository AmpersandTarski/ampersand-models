...
reden: \"Elke sessie behoort geautoriseerd te zijn op basis van de juiste DigID\"<BR>',3);
  ...
  function checkRule9(){
    // No violations should occur in (login |- aut;rol~)
    //            rule':: login/\-(aut;rol~)
    // sqlExprSrc fSpec rule':: login
     $v=DB_doquer('SELECT DISTINCT isect0.`login`, isect0.`i`
                     FROM `sessie` AS isect0
                    WHERE NOT EXISTS (SELECT *
                                 FROM 
                                    ( SELECT DISTINCT F0.`persoon`, F1.`i`
                                        FROM `aut` AS F0, `sessie` AS F1
                                       WHERE F0.`Rol`=F1.`rol`
                                    ) AS cp
                                WHERE isect0.`login`=cp.`persoon` AND isect0.`i`=cp.`i`) AND isect0.`login` IS NOT NULL AND isect0.`i` IS NOT NULL');
     if(count($v)) {
      DB_debug('Overtreding (Persoon '.$v[0][0].',Sessie '.$v[0][1].')
reden: \"De gebruiker in deze sessie dient een rol te krijgen die hij of zij conform autorisatie van de Rechtbank mag vervullen.\"<BR>',3);
      return false;
    }return true;
  }
