  ...
  class Rechtsgebied {
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      foreach($me['Procedures'] as $i0=>$v0){
        if(isset($v0['id']))
          DB_doquer("UPDATE `procedur` SET `i`='".addslashes($v0['id'])."', `proceduresoort`='".addslashes($v0['proceduresoort'])."', `zorgdrager`='".addslashes($v0['zorgdrager voor dossier'])."' WHERE `i`='".addslashes($v0['Procedure'])."'", 5);
      }
