        ...
        if(count($ctx)==0) $this->_new=true; else
        {
          ...
          foreach($me['zittingen'] as $i0=>&$v0){
            ...
            $v0['behandeling']=firstRow(DB_doquer("SELECT DISTINCT '".addslashes($v1)."' AS `id`
                                                        ...
                                                        , `f3`.`locatie` AS `locatieGerecht`
                                                        , `f4`.`kamer` AS `kamerKamer`
                                                     FROM `zitting`
                                                     LEFT JOIN `zitting` AS f3 ON `f3`.`i`='".addslashes($v1)."'
                                                     LEFT JOIN `zitting` AS f4 ON `f4`.`i`='".addslashes($v1)."'
                                                    WHERE `zitting`.`i`='".addslashes($v1)."'"));
            $v0['behandeling']['rechterPersoon']=firstCol(DB_doquer("SELECT DISTINCT `f1`.`persoon` AS `rechterPersoon`
                                                                       FROM `zitting`
      ...
      * -------------------------------------- *
      ...
      foreach($me['zittingen'] as $i0=>$v0){
        if(isset($v0['behandeling']['id']))
          DB_doquer("UPDATE `zitting` SET `i`='".addslashes($v0['behandeling']['id'])."', `locatie`='".addslashes($v0['behandeling']['locatieGerecht'])."', `kamer`='".addslashes($v0['behandeling']['kamerKamer'])."' WHERE `i`='".addslashes($v0['behandeling']['zitting'])."'", 5);
      }
      ...
      // no code for locatieGerecht,i in gerecht
      // no code for kamer,i in kamer
      // no code for kamerKamer,i in kamer
      foreach($me['machtigingen'] as $i0=>$v0){
      ...
      foreach($me['zittingen'] as $i0=>$v0){
        foreach($v0['behandeling']['rechterPersoon'] as $i2=>$v2){
      ...
      foreach($me['zittingen'] as $i0=>$v0){
        foreach  ($v0['behandeling']['rechterPersoon'] as $rechterPersoon){
          $res=DB_doquer("INSERT IGNORE INTO `rechter` (`zitting`,`persoon`) VALUES ('".addslashes($v0['behandeling']['id'])."', '".addslashes($rechterPersoon)."')", 5);
        }
      }
