<?php // generated with ADL vs. 0.8.10-452
  ...
  /********* on line 413, file "VIRO.adl"
    SERVICE Betaling : I[Document]
  ...
  class Betaling {
    ...
    private $_verzonden;
    private $_ontvangen;
    function Betaling($id=null, $zaaksdossier=null, $type=null, $van=null, $aan=null, $betreft=null, $bedrag=null, $verzonden=null, $ontvangen=null){
      $this->_id=$id;
      $this->_zaaksdossier=$zaaksdossier;
      $this->_type=$type;
      $this->_van=$van;
      $this->_aan=$aan;
      $this->_betreft=$betreft;
      $this->_bedrag=$bedrag;
      $this->_verzonden=$verzonden;
      $this->_ontvangen=$ontvangen;
      if(!isset($zaaksdossier) && isset($id)){
        // get a Betaling based on its identifier
        // check if it exists:
        ...
        if(count($ctx)==0) $this->_new=true; else
        {
          ...
          $me=firstRow(DB_doquer("SELECT DISTINCT `aan`.`document` AS `id`
                                       , `document`.`type`
                                       , `document`.`van`
                                       , `document`.`kenmerkvan` AS `betreft`
                                       , `document`.`verzonden`
                                       , `document`.`ontvangen`
                                    FROM `aan`
          ...
          $me['aan']=firstCol(DB_doquer("SELECT DISTINCT `aan`.`persoon` AS `aan`
                                           FROM `aan`
                                          WHERE `aan`.`document`='".addslashes($id)."'"));
          $me['bedrag']=firstCol(DB_doquer("SELECT DISTINCT `uwkenmerk`.`tekst` AS `bedrag`
                                              FROM `uwkenmerk`
                                             WHERE `uwkenmerk`.`document`='".addslashes($id)."'"));
          $this->set_zaaksdossier($me['zaaksdossier']);
          $this->set_type($me['type']);
          $this->set_van($me['van']);
          $this->set_aan($me['aan']);
          $this->set_betreft($me['betreft']);
          $this->set_bedrag($me['bedrag']);
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      $me=array("id"=>$this->getId(), "zaaksdossier" => $this->_zaaksdossier, "type" => $this->_type, "van" => $this->_van, "aan" => $this->_aan, "betreft" => $this->_betreft, "bedrag" => $this->_bedrag, "verzonden" => $this->_verzonden, "ontvangen" => $this->_ontvangen);
      DB_doquer("DELETE FROM `document` WHERE `i`='".addslashes($me['id'])."'",5);
      $res=DB_doquer("INSERT IGNORE INTO `document` (`type`,`van`,`kenmerkvan`,`verzonden`,`ontvangen`,`i`) VALUES ('".addslashes($me['type'])."', '".addslashes($me['van'])."', '".addslashes($me['betreft'])."', '".addslashes($me['verzonden'])."', ".((null!=$me['ontvangen'])?"'".addslashes($me['ontvangen'])."'":"NULL").", '".addslashes($me['id'])."')", 5);
      ...
      DB_doquer("DELETE FROM `tekst` WHERE `i`='".addslashes($me['betreft'])."'",5);
      foreach($me['bedrag'] as $i0=>$v0){
        DB_doquer("DELETE FROM `tekst` WHERE `i`='".addslashes($v0)."'",5);
      }
      $res=DB_doquer("INSERT IGNORE INTO `tekst` (`i`) VALUES ('".addslashes($me['betreft'])."')", 5);
      foreach($me['bedrag'] as $i0=>$v0){
      ...
      DB_doquer("DELETE FROM `uwkenmerk` WHERE `document`='".addslashes($me['id'])."'",5);
      foreach  ($me['bedrag'] as $bedrag){
        $res=DB_doquer("INSERT IGNORE INTO `uwkenmerk` (`tekst`,`document`) VALUES ('".addslashes($bedrag)."', '".addslashes($me['id'])."')", 5);
      }
      if (!checkRule1()){
    ...
    function set_betreft($val){
      $this->_betreft=$val;
    }
    function get_betreft(){
      return $this->_betreft;
    }
    function set_bedrag($val){
      $this->_bedrag=$val;
    }
    function get_bedrag(){
      if(!isset($this->_bedrag)) return array();
      return $this->_bedrag;
    }
  ...
  function getEachBetaling(){
    return firstCol(DB_doquer('SELECT DISTINCT `i`
                                 FROM `document`'));
  }

  function readBetaling($id){
      // check existence of $id
      $obj = new Betaling($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function delBetaling($id){
    $tobeDeleted = new Betaling($id);
