...
\***************************************/
  ...
  require "Rechterlijke ambtenaren.inc.php";
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['0.'.$i0]);$i0++){
      ...
      for($i1=0;isset($r['0.'.$i0.'.1.'.$i1]);$i1++){
        $Rechters[$i0]['rol'][$i1] = @$r['0.'.$i0.'.1.'.$i1.''];
      }
    }
    $Griffiers=array();
    for($i0=0;isset($r['1.'.$i0]);$i0++){
      ...
      for($i1=0;isset($r['1.'.$i0.'.1.'.$i1]);$i1++){
        $Griffiers[$i0]['rol'][$i1] = @$r['1.'.$i0.'.1.'.$i1.''];
      }
    }
  ...
  $Rechterlijkeambtenaren=new Rechterlijkeambtenaren();
    writeHead("<TITLE>Rechterlijke ambtenaren - VIRO - ADL Prototype</TITLE>"
              .($edit?'<SCRIPT type="text/javascript" src="edit.js"></SCRIPT>':'<SCRIPT type="text/javascript" src="navigate.js"></SCRIPT>')."\n" );
    if($edit)
        echo '<FORM name="editForm" action="'
              .$_SERVER['PHP_SELF'].'" method="POST" class="Edit">';
    ?><H1>Rechterlijke ambtenaren</H1>
            ...
            <LI CLASS="item UI_Rechters" ID="0.'.$i0.'">';
              ...
              <DIV>';
                ...
                foreach($v0['rol'] as $i1=>$rol){
                  echo '
                  <LI CLASS="item UI_Rechters_rol" ID="0.'.$i0.'.1.'.$i1.'">';
                    echo htmlspecialchars($rol);
                  echo '</LI>';
                }
                if($edit) echo '
                  <LI CLASS="new UI_Rechters_rol" ID="0.'.$i0.'.1.'.count($v0['rol']).'">new rol</LI>';
                echo '
                </UL>';
              echo '
              </DIV>';
          ...
          foreach($Griffiers as $i0=>$v0){
            ...
            <LI CLASS="item UI_Griffiers" ID="1.'.$i0.'">';
              ...
              <DIV>';
                ...
                foreach($v0['rol'] as $i1=>$rol){
                  echo '
                  <LI CLASS="item UI_Griffiers_rol" ID="1.'.$i0.'.1.'.$i1.'">';
                    echo htmlspecialchars($rol);
                  echo '</LI>';
                }
                if($edit) echo '
                  <LI CLASS="new UI_Griffiers_rol" ID="1.'.$i0.'.1.'.count($v0['rol']).'">new rol</LI>';
                echo '
                </UL>';
              echo '
              </DIV>';
