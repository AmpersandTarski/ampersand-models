...
\***************************************/
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    $rechtsgebied = @$r['1'];
    $proceduresoort = @$r['2'];
    $nieuweProcedure=new nieuweProcedure($ID,$zorgdrager, $rechtsgebied, $proceduresoort);
  ...
  if($nieuweProcedure){
    ...
    ?>
    <DIV class="Floater zorgdrager">
      <DIV class="FloaterHeader">zorgdrager</DIV>
      <DIV class="FloaterContent"><?php
          $zorgdrager = $nieuweProcedure->get_zorgdrager();
          echo '<SPAN CLASS="item UI_zorgdrager" ID="0">';
          if(!$edit) echo '
