<?php // generated with ADL vs. 0.8.10-452
  ...
  /********* on line 3862, file "VIRO.adl"
    SERVICE RechterlijkeAmbtenaar : I[Persoon]
   = [ gerecht : griffier~\/bezetting;gerecht
     , kamer : bezetting
        = [ kamer : [Kamer]
          , sector : sector
          ]
  ...
  class RechterlijkeAmbtenaar {
    ...
    private $_ontvangen;
    private $_verzonden;
    function RechterlijkeAmbtenaar($id=null, $gerecht=null, $kamer=null, $rol=null, $geautoriseerdvoor=null, $Zittingen=null, $ontvangen=null, $verzonden=null){
      $this->_id=$id;
      ...
      if(!isset($gerecht) && isset($id)){
        // get a RechterlijkeAmbtenaar based on its identifier
        // check if it exists:
        ...
        if(count($ctx)==0) $this->_new=true; else
        {
          ...
          // fill the attributes
          $me=array();
          $me['gerecht']=firstCol(DB_doquer("SELECT DISTINCT `f1`.`gerecht`
                                               FROM `persoon`
                                               JOIN  ( 
                                                            (SELECT DISTINCT Persoon, Gerecht AS `gerecht`
                                                                  FROM `griffier`
                                                            ) UNION (SELECT DISTINCT F0.`persoon` AS `Persoon`, F1.`gerecht`
          ...
          $me['kamer']=(DB_doquer("SELECT DISTINCT `bezetting`.`kamer` AS `id`
                                     FROM `bezetting`
    ...
    function save(){
      ...
      * -------------------------------------- *
      ...
      $me=array("id"=>$this->getId(), "gerecht" => $this->_gerecht, "kamer" => $this->_kamer, "rol" => $this->_rol, "geautoriseerd voor" => $this->_geautoriseerdvoor, "Zittingen" => $this->_Zittingen, "ontvangen" => $this->_ontvangen, "verzonden" => $this->_verzonden);
      foreach($me['Zittingen'] as $i0=>$v0){
      ...
      foreach($me['kamer'] as $i0=>$v0){
        if(isset($v0['id']))
          DB_doquer("UPDATE `kamer` SET `i`='".addslashes($v0['id'])."', `sector`='".addslashes($v0['sector'])."' WHERE `i`='".addslashes($v0['kamer'])."'", 5);
      }
      // no code for kamer,i in kamer
      foreach($me['Zittingen'] as $i0=>$v0){
        $res=DB_doquer("INSERT IGNORE INTO `kamer` (`i`,`gerecht`,`sector`) VALUES ('".addslashes($v0['kamer']['id'])."', '".addslashes($v0['kamer']['gerecht'])."', '".addslashes($v0['kamer']['sector'])."')", 5);
      ...
      foreach($me['Zittingen'] as $i0=>$v0){
        DB_doquer("DELETE FROM `sector` WHERE `i`='".addslashes($v0['kamer']['sector'])."'",5);
      }
      foreach($me['kamer'] as $i0=>$v0){
        $res=DB_doquer("INSERT IGNORE INTO `sector` (`i`) VALUES ('".addslashes($v0['sector'])."')", 5);
      }
      foreach($me['Zittingen'] as $i0=>$v0){
        $res=DB_doquer("INSERT IGNORE INTO `sector` (`i`) VALUES ('".addslashes($v0['kamer']['sector'])."')", 5);
      }
      ...
      foreach  ($me['kamer'] as $kamer){
        $res=DB_doquer("INSERT IGNORE INTO `bezetting` (`kamer`,`persoon`) VALUES ('".addslashes($kamer['id'])."', '".addslashes($me['id'])."')", 5);
      }
    ...
    function get_gerecht(){
      if(!isset($this->_gerecht)) return array();
    ...
    function get_kamer(){
      if(!isset($this->_kamer)) return array();
      return $this->_kamer;
    }
