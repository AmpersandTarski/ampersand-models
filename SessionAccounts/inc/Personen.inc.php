<?php // generated with ADL vs. 1.1.0.801
  ...
  /********* on line 47, file "F:\\RJ$\\Prive\\CC model repository\\Adlfiles\\SessionAccounts.adl"
    SERVICE Personen : I[Person]
   = [ Id : I[Person]
     , name : iscalled
     , email addr : emailOf~
     , telefoonnr : phoneOf~
  ...
  class Personen {
    ...
    private $_emailaddr;
    private $_telefoonnr;
    function Personen($id=null, $_Id=null, $_name=null, $_emailaddr=null, $_telefoonnr=null){
      $this->id=$id;
      $this->_Id=$_Id;
      $this->_name=$_name;
  ...
  function getEachPersonen(){
    return firstCol(DB_doquer('SELECT DISTINCT `I`
                                 FROM `Person`'));
  }

  function readPersonen($id){
      // check existence of $id
      $obj = new Personen($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function delPersonen($id){
    $tobeDeleted = new Personen($id);
