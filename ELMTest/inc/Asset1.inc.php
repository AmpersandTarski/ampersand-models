<?php // generated with Prototype vs. 1.1.0.874(core vs. 2.0.0.13)
  ...
  class Asset1 {
    ...
    function save(){
       ...
       if (!$this->isNew()) {
          ...
          if(isset($cutoldme['assetManager']) && count(DB_doquer("SELECT `Person` FROM `Person` WHERE `Person`='".addslashes($cutoldme['assetManager'])."'")) != 1){
             $rec = array('Person'=>null);
             $rec['Person']=$cutoldme['assetManager'];
             DB_doquer("INSERT INTO `Person` (`Person`) VALUES (".((null!=$rec['Person'])?"'".addslashes($rec['Person'])."'":"NULL").")", 5);
             if(mysql_errno()!=0) {
                $err = mysql_error();
                rollbacktransaction();
       ...
       //check the existence of attributes
       if(isset($newme['assetManager']) && count(DB_doquer("SELECT `Person` FROM `Person` WHERE `Person`='".addslashes($newme['assetManager'])."'")) != 1){
          $rec = array('Person'=>null);
          $rec['Person']=$newme['assetManager'];
          DB_doquer("INSERT INTO `Person` (`Person`) VALUES (".((null!=$rec['Person'])?"'".addslashes($rec['Person'])."'":"NULL").")", 5);
          if(mysql_errno()!=0) {
             $err = mysql_error();
             rollbacktransaction();
             $myerrors[] = print_r(array('observation'=>'you try to set an attribute to a new instance '.$newme['assetManager'].' of concept Person.','erroranalysis'=>'you can only refer to existing instances of Person on this page, because it has required attributes that are not or cannot be specified on this page.','suggestion'=>'first create '.$newme['assetManager'].' on a page that can create instances of Person.','sqlerror'=>$err)).print_r($newme);
             return false;
          }
       }
       //check the existence of attributes
  ...
  function getEachAsset1(){
    return firstCol(DB_doquer('SELECT DISTINCT `Asset`
                                 FROM `Asset`'));
  }

  function readAsset1($id){
      // check existence of $id
      $obj = new Asset1($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function delAsset1($id){
    $tobeDeleted = new Asset1($id);
