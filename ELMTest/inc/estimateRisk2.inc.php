<?php // generated with Prototype vs. 1.1.0.874(core vs. 2.0.0.13)
  ...
  /********* on line 94, file "F:\\RJ$\\Prive\\CC model repository\\Adlfiles\\ELMTest.adl"
  ...
  class estimateRisk2 {
    ...
    private $_risk;
    private $_accepted;
    function estimateRisk2($id=null,$_risk=null, $_accepted=null){
      $this->id=$id;
      $this->_risk=$_risk;
      $this->_accepted=$_accepted;
      if(!isset($_risk) && isset($id)){
        // get a estimateRisk2 based on its identifier
        // check if it exists:
  ...
  function getEachestimateRisk2(){
    return firstCol(DB_doquer('SELECT DISTINCT `Obligation`
                                 FROM `Obligation`'));
  }

  function readestimateRisk2($id){
      // check existence of $id
      $obj = new estimateRisk2($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function delestimateRisk2($id){
    $tobeDeleted = new estimateRisk2($id);
