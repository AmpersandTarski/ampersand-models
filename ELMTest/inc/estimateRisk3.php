...
\**********************/
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    $estimateRisk3=new estimateRisk3(@$_REQUEST['ID'],$accepted, $obligations);
    if($estimateRisk3->save()!==false) die('ok:'.serviceref($_REQUEST['content']).'&estimateRisk3='.urlencode($estimateRisk3->getId()));
  ...
  if(isset($_REQUEST['estimateRisk3'])){
    if(!$del || !delestimateRisk3($_REQUEST['estimateRisk3']))
      $estimateRisk3 = readestimateRisk3($_REQUEST['estimateRisk3']);
    else $estimateRisk3 = false; // delete was a succes!
  } else if($new) $estimateRisk3 = new estimateRisk3();
  else $estimateRisk3 = false;
  if($estimateRisk3){
    writeHead("<TITLE>estimateRisk3 - ctxELMtest - Ampersand Prototype</TITLE>"
    ...
    if($edit && $estimateRisk3->isNew())
         echo '<P><INPUT TYPE="TEXT" NAME="ID" VALUE="'.addslashes($estimateRisk3->getId()).'" /></P>';
    else echo '<H1>'.$estimateRisk3->getId().'</H1>';
    ?>
    <DIV class="Floater accepted">
      <DIV class="FloaterHeader">accepted</DIV>
      <DIV class="FloaterContent"><?php
          $accepted = $estimateRisk3->get_accepted();
    ...
    <DIV class="Floater obligations">
      ...
      <DIV class="FloaterContent"><?php
          $obligations = $estimateRisk3->get_obligations();
          echo '
          <UL>';
   ...
   if($edit){
     if($new) 
       ...
       $buttons.=ifaceButton("JavaScript:save('".serviceref($_REQUEST['content'])."&save=1','".urlencode($estimateRisk3->getId())."');","Save");
       $buttons.=ifaceButton(serviceref($_REQUEST['content'],false,false, array('estimateRisk3'=>urlencode($estimateRisk3->getId()) )),"Cancel");
     } 
  } else {
          $buttons.=ifaceButton(serviceref($_REQUEST['content'],false,false, array('estimateRisk3'=>urlencode($estimateRisk3->getId()),'edit'=>1)),"Edit");
          $buttons.=ifaceButton(serviceref($_REQUEST['content'],false,false, array('estimateRisk3'=>urlencode($estimateRisk3->getId()),'del'=>1)),"Delete");
         }
  }else{
    if($del){
      writeHead("<TITLE>Delete geslaagd</TITLE>");
      echo 'The estimateRisk3 is deleted';
    }else{  // deze pagina zou onbereikbaar moeten zijn
      writeHead("<TITLE>No estimateRisk3 object selected - ctxELMtest - Ampersand Prototype</TITLE>");
      ?><i>No estimateRisk3 object selected</i><?php 
    }
  }
  writeTail($buttons);
?>