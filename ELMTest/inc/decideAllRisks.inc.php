<?php // generated with Prototype vs. 1.1.0.874(core vs. 2.0.0.13)
  ...
  /********* on line 79, file "F:\\RJ$\\Prive\\CC model repository\\Adlfiles\\ELMTest.adl"
    SERVICE decideAllRisks : I[Asset];-statAssetRA
   = [ accepted : dAssetRA
  ...
  class decideAllRisks {
    protected $id=false;
    protected $_new=true;
    private $_accepted;
    private $_obligations;
    function decideAllRisks($id=null,$_accepted=null, $_obligations=null){
      $this->id=$id;
      $this->_accepted=$_accepted;
      $this->_obligations=$_obligations;
      if(!isset($_accepted) && isset($id)){
        // get a decideAllRisks based on its identifier
        // check if it exists:
  ...
  function getEachdecideAllRisks(){
  ...
  function readdecideAllRisks($id){
      // check existence of $id
      $obj = new decideAllRisks($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function deldecideAllRisks($id){
    $tobeDeleted = new decideAllRisks($id);
