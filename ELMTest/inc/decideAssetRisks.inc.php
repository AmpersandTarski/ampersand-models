<?php // generated with Prototype vs. 1.1.0.874(core vs. 2.0.0.13)
  ...
  /********* on line 70, file "F:\\RJ$\\Prive\\CC model repository\\Adlfiles\\ELMTest.adl"
    SERVICE decideAssetRisks : I[Asset];-statAssetRA
   = [ accepted : dAssetRA
     , obligations : obligationOf~
     ]
   *********/
  
  class decideAssetRisks {
    protected $id=false;
    protected $_new=true;
    private $_accepted;
    private $_obligations;
    function decideAssetRisks($id=null,$_accepted=null, $_obligations=null){
      $this->id=$id;
      $this->_accepted=$_accepted;
      $this->_obligations=$_obligations;
      if(!isset($_accepted) && isset($id)){
        // get a decideAssetRisks based on its identifier
        // check if it exists:
  ...
  function getEachdecideAssetRisks(){
  ...
  function readdecideAssetRisks($id){
      // check existence of $id
      $obj = new decideAssetRisks($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function deldecideAssetRisks($id){
    $tobeDeleted = new decideAssetRisks($id);
