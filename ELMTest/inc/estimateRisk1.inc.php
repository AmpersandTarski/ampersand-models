<?php // generated with Prototype vs. 1.1.0.874(core vs. 2.0.0.13)
  ...
  /********* on line 90, file "F:\\RJ$\\Prive\\CC model repository\\Adlfiles\\ELMTest.adl"
    SERVICE estimateRisk1 : I[Asset]
   = [ accepted : dAssetRA
  ...
  class estimateRisk1 {
    ...
    function estimateRisk1($id=null,$_accepted=null, $_obligations=null){
      $this->id=$id;
      $this->_accepted=$_accepted;
      $this->_obligations=$_obligations;
      if(!isset($_accepted) && isset($id)){
        // get a estimateRisk1 based on its identifier
        // check if it exists:
  ...
  function getEachestimateRisk1(){
    return firstCol(DB_doquer('SELECT DISTINCT `Asset`
                                 FROM `Asset`'));
  }

  function readestimateRisk1($id){
      // check existence of $id
      $obj = new estimateRisk1($id);
      if($obj->isNew()) return false; else return $obj;
  }

  function delestimateRisk1($id){
    $tobeDeleted = new estimateRisk1($id);
