<?php // generated with ADL vs. 0.8.10-485
  ...
  class Operatie {
    ...
    private $_call;
    private $_type;
    function Operatie($id=null, $_naam=null, $_call=null, $_type=null){
      $this->id=$id;
      $this->_naam=$_naam;
      $this->_call=$_call;
      $this->_type=$_type;
      if(!isset($_naam) && isset($id)){
        ...
        if(count($ctx)==0) $this->_new=true; else
        {
          ...
          $me=firstRow(DB_doquer("SELECT DISTINCT `operationtbl`.`id`
                                       ...
                                       , `operationtbl`.`call`
                                       , `operationtbl`.`outputtype` AS `type`
                                    FROM `operationtbl`
    ...
    function save(){
      ...
      * All attributes are saved *
      ...
      $me=array("id"=>$this->getId(), "naam" => $this->_naam, "call" => $this->_call, "type" => $this->_type);
      DB_doquer("DELETE FROM `operationtbl` WHERE `id`='".addslashes($me['id'])."'",5);
      $res=DB_doquer("INSERT IGNORE INTO `operationtbl` (`name`,`call`,`outputtype`,`id`) VALUES ('".addslashes($me['naam'])."', '".addslashes($me['call'])."', '".addslashes($me['type'])."', ".(!$newID?"'".addslashes($me['id'])."'":"NULL").")", 5);
      ...
      DB_doquer("DELETE FROM `outputtype` WHERE `i`='".addslashes($me['type'])."'",5);
      $res=DB_doquer("INSERT IGNORE INTO `outputtype` (`i`) VALUES ('".addslashes($me['type'])."')", 5);
      if (!checkRule1()){
