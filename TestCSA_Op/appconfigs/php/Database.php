...
{ global $allRoles;
  ...
  if (is_array($MappingEngineRules) AND in_array($ruleName, $MappingEngineRules)) continue; // Mapping rules are skipped here for performance reasons
...
{ 
    ...
    { 
        foreach ($allRoles as $role)
        { 
            if ($role['name'] == 'MappingEngine') $MappingEngineRules = $role['ruleNames'];
        } 
    }

        $error = '';

    foreach ($MappingEngineRules as $ruleName) // Loop over all rules for the role 'MappingEngine'
    { 
