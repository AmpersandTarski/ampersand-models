...
define('SMF_prefix', __DIR__ . '/SMF_'); // Prefix for semaphores
...
function SetSMF($type, $val)
{
    //Set semaphore into SMF_Pause file
    $type = preg_replace('/[^a-zA-Z0-9]/', '', $type);
    $fp = fopen(SMF_prefix . $type,'w');
    if ($fp == false) die('Could not open SMF_Pause file for writing');
    fwrite($fp,json_encode($val));
    fclose($fp);

}

function GetSMF($type)
{
    // Read semaphore from SMF_Pause file
    $type = preg_replace('/[^a-zA-Z0-9]/', '', $type);
    if (!file_exists(SMF_prefix . $type)) return false;
    return json_decode(file_get_contents(SMF_prefix . $type));
}
