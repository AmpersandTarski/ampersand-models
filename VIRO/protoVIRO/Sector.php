...
\***************************************/
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['0.'.$i0]);$i0++){
      $sectorPanel[$i0] = array( 'id' => @$r['0.'.$i0.'.0']
                               , 'Panel' => @$r['0.'.$i0.'.0']
                               , 'court' => @$r['0.'.$i0.'.1']
                               );
    }
    $Sector=new Sector($ID,$sectorPanel);
    if($Sector->save()!==false) die('ok:'.$_SERVER['PHP_SELF'].'?Sector='.urlencode($Sector->getId())); else die('');
  ...
  if($Sector){
    ...
    ?>
    <DIV class="Floater sector Panel">
      <DIV class="FloaterHeader">sector Panel</DIV>
      <DIV class="FloaterContent"><?php
          $sectorPanel = $Sector->get_sectorPanel();
          echo '
          <UL>';
          foreach($sectorPanel as $i0=>$v0){
            echo '
          ...
          if($edit) echo '
            <LI CLASS="new UI" ID="0.'.count($sectorPanel).'">new sector Panel</LI>';
          echo '
          </UL>';
        ?> 
      </DIV>
    ...
    <SCRIPT type="text/javascript">
      // code for editing blocks in sector Panel
      function UI(id){
