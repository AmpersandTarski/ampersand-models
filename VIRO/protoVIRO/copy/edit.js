...
function save(url,id){ // id can be used as an extra parameter, to set the ID value
  ...
  $.post(url,flds.join('&'),function recieveDataOnPost(data){
    ...
    if(splitdata.length==2){ // data is saved, follow URL (to exit edit mode)
	  ...
	  $('#error').slideDown();
	  $("#error").append('<a class="errorClose">x</a>');
	  $("#error").append(data);
...
function unEdit(submit,reset){ if(reset==null) reset=true;
    ...
    tabHover=editObject;
    $(editObject).empty();
    if(!submit&&editsNew){ // remove the object instead of restoring the old data
       $(editObject).remove();
       if(reset) editObject=null;
    }else{
      if(submit){ // store the data
        if(editbox.value==''){
            if($(editObject).is('li')) {
              var obj=$(editObject);
...
function editBoxIt(obj){
    editObject=obj;
    editContent=$(obj).get(0).innerHTML;
    $(obj).empty();
    $(obj).append(editbox);
    editbox.focus();
