  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['0.'.$i0]);$i0++){
      ...
      for($i1=0;isset($r['0.'.$i0.'.3.'.$i1]);$i1++){
        $Panels[$i0]['members'][$i1] = @$r['0.'.$i0.'.3.'.$i1.''];
      }
    }
            ...
            <LI CLASS="item UI" ID="0.'.$i0.'">';
              ...
              <DIV>';
                ...
                foreach($v0['members'] as $i1=>$members){
                  echo '
                  <LI CLASS="item UImembers" ID="0.'.$i0.'.3.'.$i1.'">';
                    if(!$edit){
                      echo '
                    <A class="GotoLink" id="To0.'.$i0.'.3.'.$i1.'">';
                      ...
                      echo '<LI><A HREF="InterestedParty.php?InterestedParty='.urlencode($members).'">InterestedParty</A></LI>';
                      echo '</UL></DIV>';
                    } else echo htmlspecialchars($members);
                  echo '</LI>';
                }
                if($edit) echo '
                  <LI CLASS="new UImembers" ID="0.'.$i0.'.3.'.count($v0['members']).'">new members</LI>';
                echo '
                </UL>';
              echo '
              </DIV>';
    ...
    <SCRIPT type="text/javascript">
      ...
      function UI(id){
        return '<DIV>panel: <SPAN CLASS="item UI_panel" ID="'+id+'.0"></SPAN></DIV>'
             ...
             + '<DIV>members: <UL><LI CLASS="new UI_members" ID="'+id+'.3">new members</LI></UL></DIV>'
              ;
      }
    </SCRIPT>
    <?php } ?>
