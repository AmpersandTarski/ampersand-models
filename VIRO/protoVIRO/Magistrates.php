...
\***************************************/
  ...
  if(isset($_REQUEST['save'])) { // handle ajax save request (do not show the interface)
    ...
    for($i0=0;isset($r['0.'.$i0]);$i0++){
      ...
      $Judges[$i0]['role']=array();
      for($i1=0;isset($r['0.'.$i0.'.2.'.$i1]);$i1++){
        $Judges[$i0]['role'][$i1] = @$r['0.'.$i0.'.2.'.$i1.''];
      }
    }
    $Clerks=array();
    for($i0=0;isset($r['1.'.$i0]);$i0++){
      ...
      $Clerks[$i0]['role']=array();
      for($i1=0;isset($r['1.'.$i0.'.2.'.$i1]);$i1++){
        $Clerks[$i0]['role'][$i1] = @$r['1.'.$i0.'.2.'.$i1.''];
      }
    }
          ...
          foreach($Judges as $i0=>$v0){
            ...
            <LI CLASS="item UI_Judges" ID="0.'.$i0.'">';
              ...
              <DIV>';
                ...
                echo '
                <UL>';
                foreach($v0['role'] as $i1=>$role){
                  echo '
                  <LI CLASS="item UI_Judges_role" ID="0.'.$i0.'.2.'.$i1.'">';
                    echo htmlspecialchars($role);
                  echo '</LI>';
                }
                if($edit) echo '
                  <LI CLASS="new UI_Judges_role" ID="0.'.$i0.'.2.'.count($v0['role']).'">new role</LI>';
                echo '
                </UL>';
              echo '
              </DIV>';
    ...
    <SCRIPT type="text/javascript">
      ...
      function UI_Judges(id){
        return '<DIV>name: <SPAN CLASS="item UI_Judges_name" ID="'+id+'.0"></SPAN></DIV>'
             ...
             + '<DIV>role: <UL><LI CLASS="new UI_Judges_role" ID="'+id+'.2">new role</LI></UL></DIV>'
              ;
      }
    </SCRIPT>
    <?php } ?>
          ...
          foreach($Clerks as $i0=>$v0){
            ...
            <LI CLASS="item UI_Clerks" ID="1.'.$i0.'">';
              ...
              <DIV>';
                ...
                foreach($v0['role'] as $i1=>$role){
                  echo '
                  <LI CLASS="item UI_Clerks_role" ID="1.'.$i0.'.2.'.$i1.'">';
                    echo htmlspecialchars($role);
                  echo '</LI>';
                }
                if($edit) echo '
                  <LI CLASS="new UI_Clerks_role" ID="1.'.$i0.'.2.'.count($v0['role']).'">new role</LI>';
                echo '
                </UL>';
              echo '
              </DIV>';
    ...
    <SCRIPT type="text/javascript">
      ...
      function UI_Clerks(id){
        return '<DIV>name: <SPAN CLASS="item UI_Clerks_name" ID="'+id+'.0"></SPAN></DIV>'
             ...
             + '<DIV>role: <UL><LI CLASS="new UI_Clerks_role" ID="'+id+'.2">new role</LI></UL></DIV>'
              ;
      }
    </SCRIPT>
    <?php } ?>
